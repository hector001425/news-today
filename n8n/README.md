...
`pulso-ia-pipeline.json` es una plantilla importable en n8n con el flujo completo:
...
Cada 6h → buscar noticias de IA → deduplicar → por cada noticia:
    Claude resume y traduce al español → arma Markdown con frontmatter
        ├─→ commit a GitHub (pulso-ia/src/content/articles/) → Cloudflare Pages redespliega solo
        └─→ genera audio (TTS) → genera video corto vertical → publica en redes sociales
```

## Cómo importarlo

1. En tu instancia de n8n: **Workflows → Import from File** → seleccioná `pulso-ia-pipeline.json`.
...
| Publicar en GitHub | GitHub | Token con permiso `repo` sobre `pulso-ia` |
...
1. Conectá el repo `pulso-ia` a Cloudflare Pages (Dashboard → Workers & Pages → Create → Pages → Connect to Git).
2. Build command: `npm run build` — Output directory: `dist`.
3. Cada commit que el nodo de GitHub haga a `main` dispara un redespliegue automático — no hace falta tocar nada más.
